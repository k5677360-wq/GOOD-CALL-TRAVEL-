...
function renderDestinations() {
    ...
    const dotsContainer = document.getElementById('magDots');
    let current = 0;
    const total = destinationsData.length;
    const cardWidth = 70; // porcentaje

    // Crear dots
    if (dotsContainer) {
        dotsContainer.innerHTML = destinationsData.map((_, i) =>
            `<button class="mag-dot ${i === 0 ? 'active' : ''}" data-i="${i}"></button>`
        ).join('');

        dotsContainer.querySelectorAll('.mag-dot').forEach(dot => {
    ...
    function goTo(index) {
        ...
        const offset = current * (cardWidth + 0.15);
        track.style.transform = `translateX(-${offset}%)`;
        document.querySelectorAll('.mag-dot').forEach((d, i) => {
            d.classList.toggle('active', i === current);
        });
    }

    document.getElementById('magNext')?.addEventListener('click', () => goTo(current + 1));
    document.getElementById('magPrev')?.addEventListener('click', () => goTo(current - 1));
