...
function leerParametrosURL() {
    ...
    parametrosBusqueda = {
        // Soporte para parámetros en inglés (index.html) y español (legacy)
        origen: urlParams.get('origen') || urlParams.get('origin'),
        destino: urlParams.get('destino') || urlParams.get('destination'),
        fecha: urlParams.get('fecha') || urlParams.get('date'),
        pasajeros: parseInt(urlParams.get('pasajeros') || urlParams.get('adults')) || 1
    };

    // Si la fecha viene con guiones (YYYY-MM-DD), quitarle los guiones para la API
    if (parametrosBusqueda.fecha && parametrosBusqueda.fecha.includes('-')) {
        parametrosBusqueda.fecha = parametrosBusqueda.fecha.replace(/-/g, '');
    }

    if (!parametrosBusqueda.origen || !parametrosBusqueda.destino || !parametrosBusqueda.fecha) {
        console.warn('Parámetros faltantes:', parametrosBusqueda);
        mostrarError('Faltan datos de búsqueda. Por favor vuelve al inicio.');
    }
}
...
async function buscarVuelos() {
    try {
        ...
        let destinoIATA = parametrosBusqueda.destino;

        if (origenIATA.length > 3) {
            origenIATA = obtenerCodigoIATA(origenIATA) || origenIATA;
        }
        if (destinoIATA.length > 3) {
            destinoIATA = obtenerCodigoIATA(destinoIATA) || destinoIATA;
        }

        console.log(`🔎 Buscando vuelos: ${origenIATA} -> ${destinoIATA} (${parametrosBusqueda.fecha})`);
        ...
        if (resultado.success && resultado.vuelos && resultado.vuelos.length > 0) {
            ...
            const errorMsg = resultado.error || 'No se pudieron obtener los vuelos';
...
function crearCardVuelo(vuelo, index) {
    ...
    const totalGrupoUSD = precioUSD * parametrosBusqueda.pasajeros;
    ...
    <div class="vuelo-card" id="card-${index}">
        ...
        <!-- EQUIPAJE RÁPIDO (PILLS) -->
        <div class="card-equipaje">
            <div class="equip-item ${eqPersonalIncluido ? 'equip-incluido' : 'equip-no'}">
                <span class="equip-icon">🎒</span> Bolso personal: ${eqPersonalIncluido ? 'Incluido' : 'No incluido'}
            </div>
            <div class="equip-item ${eqManoIncluido ? 'equip-incluido' : 'equip-no'}">
                <span class="equip-icon">💼</span> Mano: ${eqManoIncluido ? 'Incluido' : 'No incluido'}
            </div>
            <div class="equip-item ${eqBodegaIncluido ? 'equip-incluido' : 'equip-no'}">
                <span class="equip-icon">🧳</span> Bodega: ${eqBodegaIncluido ? 'Incluido' : 'No incluido'}
            </div>
        </div>
...
function reservar(index) {
    ...
    const precioUSD = precioFinalUSD; // aseguramos
